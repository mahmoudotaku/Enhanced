...
def get_info():
    """Legacy route for getting video info"""
    ...
    return jsonify({
        'success': True,
        'title': video_info.get('title', 'غير محدد')[:60],
        'duration': format_duration(video_info.get('duration', 0)),
        'size': format_file_size(video_info.get('filesize') or video_info.get('filesize_approx', 0))
    })

@app.route('/download', methods=['POST'])
def download():
    """Legacy route for direct download"""
    url = request.form.get('url', '').strip()
    quality = request.form.get('quality', 'best')
    
    if not url:
        flash('لم يتم توفير رابط الفيديو', 'error')
        return redirect(url_for('index'))
    
    # Start download process
    download_id = generate_download_id()
    
    # For legacy support, we'll do synchronous download
    try:
        download_result = download_video(url, quality, download_id)
        if download_result['success']:
            ...
            flash(f'خطأ في التحميل: {download_result["error"]}', 'error')
    except Exception as e:
        flash(f'خطأ في التحميل: {str(e)}', 'error')
